...
Information about orders
    ...
    COMMANDS:
       ...
       bulk, b        Bulk edit orders from a CSV spreadsheet
       help, h        Shows a list of commands or help for one command
...
You can bulk edit multiple orders in a single spreadsheet. The supported format is a combination of Shopify's Order Export CSV:

Bulk order editing is tricky business because it depends on a variety of order and line items states and permissions. We outline some common cases below.
The credentials you provide must have access to read customers, edit orders, and read+write fulfillments.

##### Identifying Orders and Line Items to Edit

Orders can be identified by Name or ID. "Name" is the user-friendly order numbers generated by Shopify. ID can be the order's integer id or its Global ID.

Order lines are grouped by their order's identifier. A line can be identified by its ID (integer or Global), product SKU, or product barcode.

##### Changing an Order's Customer

Customers are identified by their ID (integer or Global ID) or their email address. If more than 1 customer has that address an error occurs.

To change the customer simply add [the desired order identifier](#Identifying-Orders-and-Line-Items-to-Edit) column and desired customer column (Customer ID or Email).
If the customer in the spreadsheet differs form that of the order it will be changed.
